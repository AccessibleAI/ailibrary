...
class SKTrainerClassification:
	...
	def __train_with_cv(self):
		...
		def get_train_loss_type(shortcut_name):
			...
			if shortcut_name == 'F1':
				...
				raise Exception('Train loss type for cross validation can be '
								'one of: F1, LOG, MSE, RMSE, MAE, R2.')
	...
	def get_loss_function(shortcut_name):
		...
		if shortcut_name == 'F1':
			...
			raise Exception('loss type can be one of: F1, LOG, MSE, MAE, R2, zero_one_loss.')
